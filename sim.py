...
頭脳: random / rules / table:<ラウンド> / laya (未学習) / laya:<世代名>
      末尾に @<鋭さ> を付けると行動の引き方を変えられる (@max で常に最有力、既定は 2.5)
例:   uv run sim.py 40 800 random rules table:8 table:8@max laya laya:gen8
"""
import json
import random
import statistics
...
def play(brain, seed, max_turns):
    ...
    while not g.dead and g.turn < max_turns:
        ...
        g.step(d["action"])
        g.log.clear()
    return g.depth, g.kills, g.dead, g.turn, ms, miss / max(1, n)
...
def report(name, results, dt):
    ...
    print(f"[{name:16s}] 到達階 平均 {statistics.mean(depth):.2f} (最高 {max(depth)}) | 撃破 {statistics.mean(r[1] for r in results):5.1f} | "
          f"死亡 {sum(r[2] for r in results)}/{len(results)} | 生存ターン {statistics.mean(r[3] for r in results):4.0f}"
...
def main():
    ...
    for spec in specs:
        ...
        if spec.startswith("laya"):
            ...
            name, sharpness = split_sharpness(spec)
            gen = name[5:] or None
            if laya is None:
                laya = LayaBrain(gen)
            else:
