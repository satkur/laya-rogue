...
改めて Snake デモのコードを読むと、プランナーが各方向に `Safe. Best route to food.` のような答えを書き、Laya はそれを読んで選んでいるだけの出来レースだった。[紹介記事](https://zenn.dev/mizchi/articles/laya-mlx-60fps)でも「判断の質は説明文の質に依存します。」「正直、ここには限界を感じました。」と書かれている。
