...
- On that earlier pilot, "explore_fully" was worse at every depth tested (wandering monsters arrive faster than the experience helps; a level-1 hero exploring level 1-2 fully is killed by hobgoblins), and forcing "descend_asap" everywhere was also worse than leaving it free (it skips items and easy experience). That pilot explored on its own and this one does not, so neither result transfers; the plan's effect on this pilot is not measured yet.
...
- A strategist that used "explore_fully" and "fight" liberally scored 4.5 on an earlier pilot; one that left everything free scored about the same as the pilot alone. Every constraint replaces the pilot's judgement, so set one only when you can say why the pilot's default would be wrong here.
